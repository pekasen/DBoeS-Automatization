...
class TestParliamentList(unittest.TestCase):
	...
	def test_c_extracted_parlamentarians(self):
		fetcher = Wikifetcher()
		for _, parliament_data in parliaments.items():
			politicians_table = fetcher.get_politicians(parliament_data["url"], parliament_data["index"])
			# expect at least 50 politicians per parliament
			n = len(politicians_table.index)
			self.assertGreaterEqual(n, 50, "Extracted only %d entries from table %d at %s" % (n, parliament_data["index"], parliament_data["url"]))
